...
if __name__ =="__main__":
	...
	umls = build_data(name = 'umls',path = tools.cur_path + '/datasets/')


	#SGD hyper-parameters:
	params = Parameters(learning_rate = 0.5, 
						max_iter = 1000, 
						batch_size = int(len(umls.train.values) / 10),  #Make 100 batches
						neg_ratio = 1, 
						valid_scores_every = 50,
						learning_rate_policy = 'adagrad',
						contiguous_sampling = False )

	all_params = { "HTransRESCAL_Logistic_Model" : params } ; emb_size = 200; lmbda =0.5
	...
	umls.grid_search_on_all_models(all_params, embedding_size_grid = [emb_size], lmbda_grid = [lmbda], nb_runs = 1)

	#Print best averaged metrics:
	umls.print_best_MRR_and_hits()