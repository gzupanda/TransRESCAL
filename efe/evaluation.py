...
class Scorer(object):
	...
	def compute_scores(self, model, model_s, params, eval_set):
		...
		if self.compute_ranking_scores:
			...
			if model_s.startswith("HTransRescal") or model_s.startswith("TTransRescal"):
				#Fast super-ugly filtered metrics computation for Complex, DistMult, RESCAL and TransE
				logger.info("Fast MRRs")

				def htransrescal_eval_o(i,j):
					return ((e[i,:] - r[j,:]).dot(R[j,:,:])).dot((e+r[j,:]).T)
				def htransrescal_eval_s(j,k):
					return (e-r[j,:]).dot(R[j,:,:].dot(e[k,:]+r[j,:]))
				def ttransrescal_eval_o(i,j):
					return ((e[i,:] + r[j,:]).dot(R[j,:,:])).dot((e-r[j,:]).T)
				def ttransrescal_eval_s(j,k):
					return (e+r[j,:]).dot(R[j,:,:].dot(e[k,:]-r[j,:]))
				
				if model_s.startswith("HTransRESCAL"):
					e = model.e.get_value(borrow=True)
					r = model.r.get_value(borrow=True)
					R = model.R.get_value(borrow=True)
					eval_o = htransrescal_eval_o
					eval_s = htransrescal_eval_s

				if model_s.startswith("TTransRESCAL"):
					e = model.e.get_value(borrow=True)
					r = model.r.get_value(borrow=True)
					R = model.R.get_value(borrow=True)
					eval_o = ttransrescal_eval_o
					eval_s = ttransrescal_eval_s
