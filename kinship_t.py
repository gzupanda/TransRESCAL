...
if __name__ =="__main__":
	...
	kinship = build_data(name = 'kinship',path = tools.cur_path + '/datasets/')


	#SGD hyper-parameters:
	params = Parameters(learning_rate = 0.5, 
						max_iter = 1000, 
						batch_size = int(len(kinship.train.values) / 20),  #Make 100 batches
						neg_ratio = 1, 
	...
	all_params = { "TTransRESCAL_Logistic_Model" : params } ; emb_size = 10; lmbda =0.02
	...
	kinship.grid_search_on_all_models(all_params, embedding_size_grid = [emb_size], lmbda_grid = [lmbda], nb_runs = 1)

	#Print best averaged metrics:
	kinship.print_best_MRR_and_hits()
